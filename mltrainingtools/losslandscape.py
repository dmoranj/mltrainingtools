...
def select_directions(model):
    ...
    for parameter in model.named_parameters():
        ...
        normalized_direction = direction * parameter[1].norm().detach().numpy()
...
def perturb(base_parameters, north, south, i, j, step_size):
    ...
    for name, parameter in base_parameters():
        new_value = parameter.detach().numpy() + i*step_size*north[name] + j*step_size*south[name]
...
def estimate_loss(model, loss, dataset, new_parameters, max_batch=5):
    ...
    for i, data in enumerate(dataset, 0):
        ...
        accumulated_loss.append(loss_value.detach().numpy()/batch_size)
...
def generate_loss_grid(model, loss, dataset, north, south, step_size=0.1, steps=10):
    ...
    for i in range(steps):
        ...
        for j in range(steps):
            ...
            loss_value = estimate_loss(model, loss, dataset, new_parameters)
...
def train(model, criterion, optimizer, ds_train):
    running_loss = 0.0
...
def execute():
    ...
    # TODO: add everything to CUDA

    log("Load dataset")
    ds_train, ds_test = load_dataset()

    log("Load network")
    model, loss, optimizer = load_network()

    log("Train network")
    model = train(model, loss, optimizer, ds_train)
    ...
    grid = generate_loss_grid(model, loss, ds_test, north, south)
